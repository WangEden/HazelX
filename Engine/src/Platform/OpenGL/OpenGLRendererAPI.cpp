...
namespace Ripple {
	...
	static void OpenGLLogMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
	{
		switch (severity)
		{
			case GL_DEBUG_SEVERITY_HIGH:
				RP_CORE_ERROR("[OpenGL Debug HIGH] {0}", message);
				RP_CORE_ASSERT(false, "GL_DEBUG_SEVERITY_HIGH");
				break;
			case GL_DEBUG_SEVERITY_MEDIUM:
				RP_CORE_WARN("[OpenGL Debug MEDIUM] {0}", message);
				break;
			case GL_DEBUG_SEVERITY_LOW:
				RP_CORE_INFO("[OpenGL Debug LOW] {0}", message);
				break;
			case GL_DEBUG_SEVERITY_NOTIFICATION:
				// RP_CORE_TRACE("[OpenGL Debug NOTIFICATION] {0}", message);
				break;
		}
	}
	...
	void RendererAPI::Init()
	{
		...
		{
			RP_CORE_ERROR("OpenGL Error: {0}", error);
			error = glGetError();
		}
