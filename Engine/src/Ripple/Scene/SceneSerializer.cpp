...
namespace Ripple {
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		RP_CORE_INFO("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				RP_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (transformComponent)
				{
					...
					RP_CORE_INFO("  Entity Transform:");
					RP_CORE_INFO("    Translation: {0}, {1}, {2}", translation.x, translation.y, translation.z);
					RP_CORE_INFO("    Rotation: {0}, {1}, {2}, {3}", rotation.w, rotation.x, rotation.y, rotation.z);
					RP_CORE_INFO("    Scale: {0}, {1}, {2}", scale.x, scale.y, scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					RP_CORE_INFO("  Script Module: {0}", moduleName);
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								switch (type)
								{
									...
									{
										RP_CORE_ASSERT(false, "Unimplemented");
										break;
									}
									case FieldType::Vec2:
									{
				...
				if (meshComponent)
				{
					...
					RP_CORE_INFO("  Mesh Asset Path: {0}", meshPath);
				}
				...
				if (cameraComponent)
				{
					...
					RP_CORE_INFO("  Primary Camera: {0}", component.Primary);
				}
				...
				if (spriteRendererComponent)
				{
					...
					RP_CORE_INFO("  SpriteRendererComponent present.");
				}
			}
		}
		return true;
