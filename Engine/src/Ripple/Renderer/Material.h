...
// 材质是着色器、Uniforms、Textures等渲染状态的集合，
// 定义了一个物体在渲染时应该使用哪些资源和参数。
// 它是一个抽象的概念，可以根据具体的渲染需求进行设计和实现。

// 代码中采用了 材质模版 (Material) 和 材质实例 (MaterialInstance) 的双层架构。
// 这是一种工业界标准设计（类似于 Unreal Engine），旨在提高渲染效率
...
namespace Ripple {
	...
	public:
		...
		void Set(const std::string& name, const T& value)
		{
			...
			// RP_CORE_ASSERT(decl, "Could not find uniform with name '{0}'", name);
			RP_CORE_ASSERT(decl, "Could not find uniform with name 'x'");
	...
	public:
		...
		void Set(const std::string& name, const Ref<Texture>& texture)
		{
			...
			if (!decl)
				RP_CORE_WARN("Cannot find material property: ", name);
			uint32_t slot = decl->GetRegister();
			if (m_Textures.size() <= slot)
				m_Textures.resize((size_t)slot + 1);
			m_Textures[slot] = texture;
