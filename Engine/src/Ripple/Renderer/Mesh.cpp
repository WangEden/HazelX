...
#if MESH_DEBUG_LOG
#define RP_MESH_LOG(...) RP_CORE_TRACE(__VA_ARGS__)
#else
#define RP_MESH_LOG(...)
#endif
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			RP_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		RP_CORE_INFO("Loading mesh: {0}", filename.c_str());

		m_Importer = std::make_unique<Assimp::Importer>();

		const aiScene* scene = m_Importer->ReadFile(filename, s_MeshImportFlags);
		if (!scene || !scene->HasMeshes())
			RP_CORE_ERROR("Failed to load mesh file: {0}", filename);

		m_Scene = scene;

		m_IsAnimated = scene->mAnimations != nullptr;
		m_MeshShader = m_IsAnimated ? Renderer::GetShaderLibrary()->Get("RipplePBR_Anim") : Renderer::GetShaderLibrary()->Get("RipplePBR_Static");
		//m_Material.reset(new Ripple::Material(m_MeshShader));
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			RP_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			RP_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				RP_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						RP_MESH_LOG("Found existing bone in map");
		...
		{
			RP_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				RP_MESH_LOG("  {0} (Index = {1})", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				RP_MESH_LOG("    TextureCount = {0}", textureCount);
				...
				RP_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				RP_MESH_LOG("    ROUGHNESS = {0}", roughness);
				bool hasAlbedoMap = aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &aiTexPath) == AI_SUCCESS;
				if (hasAlbedoMap)
				{
					// TODO: Temp - this should be handled by Ripple's filesystem
					std::filesystem::path path = filename;
					auto parentPath = path.parent_path();
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					RP_MESH_LOG("    Albedo map path = {0}", texturePath);
					...
					{
						RP_CORE_ERROR("Could not load texture: {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					RP_MESH_LOG("    Normal map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					RP_MESH_LOG("    Roughness map path = {0}", texturePath);
					auto texture = Texture2D::Create(texturePath);
					if (texture->Loaded())
					{
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						RP_MESH_LOG("    Metalness map path = {0}", texturePath);
				...
				{
					RP_MESH_LOG("    No metalness texture");
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					RP_MESH_LOG("Material Property:");
					RP_MESH_LOG("  Name = {0}", prop->mKey.data);
					// RP_MESH_LOG("  Type = {0}", prop->mType);
					// RP_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					RP_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						RP_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						RP_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						RP_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						RP_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						RP_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						RP_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						RP_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						RP_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						RP_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						RP_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						RP_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						RP_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						RP_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// RP_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
	...
	{
		RP_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		RP_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		RP_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		RP_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		RP_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		RP_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		RP_MESH_LOG("------------------------------------------------------");
		RP_MESH_LOG("Vertex Buffer Dump");
		RP_MESH_LOG("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				RP_MESH_LOG("Vertex: {0}", i);
				RP_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				RP_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				RP_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				RP_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				RP_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				RP_MESH_LOG("--");
			}
		}
		else
