...
namespace Ripple {
	...
	void SceneRenderer::Init()
	{
		...
		geoRenderPassSpec.TargetFramebuffer = Ripple::Framebuffer::Create(geoFramebufferSpec);
		...
		compRenderPassSpec.TargetFramebuffer = Ripple::Framebuffer::Create(compFramebufferSpec);
	...
	{
		RP_CORE_ASSERT(!s_Data.ActiveScene, "");
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> SceneRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		RP_CORE_ASSERT(envEquirect->GetFormat() == TextureFormat::Float16, "Texture is not HDR!");
	...
	void SceneRenderer::FlushDrawList()
	{
		...
		s_Data.DrawList.clear(); // 清除绘制列表以准备下一帧
		s_Data.SelectedMeshDrawList.clear(); // 清除选中网格的绘制列表
		s_Data.SceneData = {};
	}
