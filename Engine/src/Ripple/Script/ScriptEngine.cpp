...
namespace Ripple {
	...
	static void InitMono()
	{
		...
		auto domain = mono_jit_init("Ripple");

		char* name = (char*)"RippleRuntime";
	...
	void ScriptEngine::LoadRippleRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("Ripple Runtime", nullptr);
	...
	{
		LoadRippleRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadRippleRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		RP_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
	...
	static FieldType GetRippleFieldType(MonoType* monoType)
	{
		...
		case MONO_TYPE_VALUETYPE:
		{
			...
			if (strcmp(name, "Ripple.Vector2") == 0) return FieldType::Vec2;
			if (strcmp(name, "Ripple.Vector3") == 0) return FieldType::Vec3;
			if (strcmp(name, "Ripple.Vector4") == 0) return FieldType::Vec4;
		}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		// Retrieve public fields (TODO: cache these fields if the module is used more than once)
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				FieldType rippleFieldType = GetRippleFieldType(fieldType);
				...
				{
					PublicField field = { name, rippleFieldType };
	...
	EntityInstanceData& ScriptEngine::GetEntityInstanceData(UUID sceneID, UUID entityID)
	{
		...
		RP_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	{
		RP_CORE_ASSERT(m_EntityInstance->GetInstance());
