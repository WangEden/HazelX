...
#include "Ripple/ImGui/ImGuizmo.h"
#include "Ripple/Renderer/Renderer2D.h"
...
namespace Ripple {
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (m_GizmoType != -1 && m_SelectionContext.size())
		{
			...
			bool snap = Input::IsKeyPressed(RP_KEY_LEFT_CONTROL);
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Open Scene..."))
				{
					...
					std::string filepath = app.OpenFile("Ripple Scene (*.hsc)\0*.hsc\0");
					if (!filepath.empty())
					{
				...
				{
					auto& app = Application::Get();
					std::string filepath = app.SaveFile("Ripple Scene (*.hsc)\0*.hsc\0");
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(RP_BIND_EVENT_FN(EditorLayer::OnKeyPressedEvent));
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(RP_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::G:
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == RP_MOUSE_BUTTON_LEFT && !Input::IsKeyPressed(KeyCode::LeftAlt) && !ImGuizmo::IsOver())
		{
					...
					for (uint32_t i = 0; i < submeshes.size(); i++)
					{
						...
						if (intersects)
						{
							...
							for (const auto& triangle : triangleCache)
							{
								...
								{
									RP_WARN("INTERSECTION: {0}, t={1}", submesh.NodeName, t);
