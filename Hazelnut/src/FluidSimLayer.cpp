...
//					// OrthographicCamera 没有SetProjection，需要重新创建
//					m_Camera = OrthographicCamera(-orthoSize * aspectRatio, orthoSize * aspectRatio, -orthoSize, orthoSize);
//				}
//			}
//
...
//			if (m_Obstacle.Enabled) {
//				// 用 Quad 模拟圆形
//				float obstacleSize = m_Obstacle.Radius * 2.0f;
//				glm::vec2 obstaclePos = m_Obstacle.Center - glm::vec2(m_Obstacle.Radius);
//				Renderer2D::DrawQuad(obstaclePos, { obstacleSize, obstacleSize }, { 0.8f, 0.2f, 0.2f, 1.0f });
//			}
//
//			for (const auto& p : m_Particles) {
//				float colorVal = std::clamp(p.Density / m_TargetDensity, 0.5f, 1.5f);
//				glm::vec2 particlePos = p.Position - glm::vec2(m_ParticleRadius);
//				float particleSize = m_ParticleRadius * 2.0f;
//				// 用小方块模拟粒子
//				Renderer2D::DrawQuad(particlePos, { particleSize, particleSize }, { 0.2f * colorVal, 0.6f * colorVal, 1.0f, 1.0f });
//			}
//
//			if (isLeftPressed || isRightPressed) {
//				float cursorSize = m_InteractionRadius / 2.0f;
//				glm::vec2 cursorPos = mouseWorldPos - glm::vec2(cursorSize / 2.0f);
//				Renderer2D::DrawQuad(cursorPos, { cursorSize, cursorSize }, { 0.2f, 0.3f, 0.5f, 0.7f });
//			}
//
//			Renderer2D::EndScene();
